...
var PORT = 3e3;
var JWT_SECRET = process.env.JWT_SECRET || "affiliate_default_secure_jwt_secret_2026_key";
var ADMIN_PASSCODE = process.env.ADMIN_PASSCODE || "admin123456";
...
var firestoreDb = getFirestore(fbApp, "ai-studio-viprewardsgaming-107607cc-236e-4fc6-91c7-209f529cf75b");
...
app.get("/api/cdn/images/:platformId.webp", async (req, res) => {
  ...
  try {
    ...
    const webpBuffer = await sharp(buffer).resize({ width: 128, withoutEnlargement: true }).webp({ quality: 80 }).toBuffer();
...
async function startServer() {
  ...
  if (!isProduction) {
    ...
    const distPath = candidates.find((c) => fs.existsSync(path.join(c, "index.html")) && fs.existsSync(path.join(c, "assets"))) || candidates.find((c) => fs.existsSync(path.join(c, "index.html"))) || path.join(process.cwd(), "dist");
