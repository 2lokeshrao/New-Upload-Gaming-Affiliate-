...
async function readDataFile() {
  try {
    if (!fs.existsSync(DATA_FILE)) {
      await fs_promises.writeFile(DATA_FILE, JSON.stringify({
        platforms: {},
        settings: {},
        custom_pages: {},
        sub_partners: {}
    ...
    const raw = await fs_promises.readFile(DATA_FILE, 'utf8');
...
async function writeDataFile(data) {
  try {
    await fs_promises.writeFile(DATA_FILE, JSON.stringify(data, null, 2));
  } catch (e) {
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  serverTs = serverTs.replace("import mysql from 'mysql2/promise';", "import fs_promises from 'fs/promises';");
  
  fs.writeFileSync('server.ts', serverTs);
  console.log('Successfully patched server.ts to use app_data.json instead of MySQL');
} else {
