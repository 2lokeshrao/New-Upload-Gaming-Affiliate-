...
Using the promo code **BONUS20OFF** gives you an instant 20% discount on your hosting plan.`
  ...
  {
    id: "thunderpick",
    postbackKey: "thunder_sec_k9X2mP",
    slug: "thunderpick",
    name: "Thunderpick",
...
var stateTrackLogs = [];
var DATA_FILE = import_path.default.join(process.cwd(), "app_data.json");
var firebaseConfig = {
...
var firestoreDb = (0, import_firestore.getFirestore)(fbApp, "ai-studio-viprewardsgaming-107607cc-236e-4fc6-91c7-209f529cf75b");
async function setDoc(coll, docId, data) {
  try {
    const cleanData = JSON.parse(JSON.stringify(data));
    await (0, import_firestore.setDoc)((0, import_firestore.doc)(firestoreDb, coll, docId), cleanData);
  } catch (e) {
...
async function getCollection(coll) {
  try {
    const querySnapshot = await (0, import_firestore.getDocs)((0, import_firestore.collection)(firestoreDb, coll));
    const docs = [];
    querySnapshot.forEach((doc2) => docs.push(doc2.data()));
    return docs;
  } catch (e) {
...
async function getDoc(coll, docId) {
  try {
    const docSnap = await (0, import_firestore.getDoc)((0, import_firestore.doc)(firestoreDb, coll, docId));
...
async function loadState() {
  try {
    ...
    if (spSnap.length > 0) stateSubPartners = spSnap;
    const cpSnap = await getCollection("custom_pages");
    stateCustomPages = cpSnap;
...
function setupRealtimeListeners() {
  (0, import_firestore.onSnapshot)((0, import_firestore.collection)(firestoreDb, "platforms"), (snapshot) => {
  ...
  (0, import_firestore.onSnapshot)((0, import_firestore.doc)(firestoreDb, "settings", "globalConfig"), (snapshot) => {
    if (snapshot.exists()) {
...
function getFilteredPlatforms(req, geo) {
  ...
  return statePlatforms.filter((p) => {
    ...
    const { apiKey, postbackKey, affiliateId, partnerApiUrl, ...safeP } = p;
    return {
      ...safeP,
...
var handleAdminLogin = (req, res) => {
  ...
  if (password === ADMIN_PASSCODE) {
    ...
    loginAttemptTracker[clientIp] = { attempts: [], lockUntil: 0 };
    const token = import_jsonwebtoken.default.sign({ role: "demo", authAt: Date.now() }, JWT_SECRET, { expiresIn: "8h" });
...
app.get("/api/admin/data", verifyJwtToken, (req, res) => {
  ...
  let returnedPlatforms = isDemo ? statePlatforms.map((p) => {
    const { postbackKey, apiKey, partnerApiUrl, affiliateId, ...safe } = p;
    return safe;
  }) : statePlatforms;
  let returnedStats = stateStats;
  let returnedSubPartners = stateSubPartners;
  let returnedConfig = JSON.parse(JSON.stringify(stateConfig));
  if (isDemo) {
    const dummyPanelConfigs = statePlatforms.map((p) => ({
      platformId: p.id,
      platformName: p.name,
      apiKey: `DEMO_KEY_${Math.floor(Math.random() * 9e5)}`,
      partnerApiUrl: `https://api.${p.slug}.com/v1/stats`,
      affiliateId: `DEMO_ID_${Math.floor(Math.random() * 9e3)}`,
      postbackKey: `pb_demo_secret`,
      syncEnabled: true,
      lastSyncedAt: (/* @__PURE__ */ new Date()).toISOString(),
      stats: {
        totalRegistrations: Math.floor(Math.random() * 5e3) + 1e3,
        ftdCount: Math.floor(Math.random() * 2e3) + 500,
        totalDepositsAmount: Math.floor(Math.random() * 5e4) + 1e4,
        netGamingRevenue: Math.floor(Math.random() * 2e4) + 5e3,
        commissionEarned: Math.floor(Math.random() * 8e3) + 1e3,
        revSharePercent: 50
      }
    }));
    returnedConfig.partnerPanelConfigs = dummyPanelConfigs;
    returnedStats = {
      totalVisits: Math.floor(Math.random() * 5e5) + 2e5,
      totalClicks: Math.floor(Math.random() * 1e5) + 5e4,
      totalPromoCopies: Math.floor(Math.random() * 3e4) + 1e4,
      totalSubPartnerApps: 3,
      platformStats: stateStats.platformStats || {},
      totalOutboundClicks: Math.floor(Math.random() * 1e5) + 5e4,
      activeUsersLive: Math.floor(Math.random() * 150) + 20,
      dailyTrends: [
        { date: "Aug 20", clicks: 2e3, conversions: 150 },
        { date: "Aug 21", clicks: 2500, conversions: 180 },
        { date: "Aug 22", clicks: 2100, conversions: 160 },
        { date: "Aug 23", clicks: 3e3, conversions: 220 },
        { date: "Aug 24", clicks: 3500, conversions: 280 },
        { date: "Aug 25", clicks: 4200, conversions: 350 },
        { date: "Aug 26", clicks: 5e3, conversions: 400 }
      ]
    };
    returnedSubPartners = [
      { id: "demo1", fullName: "Rahul Sharma", email: "rahul.s@example.com", whatsapp: "919876543210", platformId: "10cric", platformName: "10CRIC", trafficSource: "Telegram", estimatedMonthlyPlayers: "100-500", status: "approved", appliedAt: (/* @__PURE__ */ new Date()).toISOString() },
      { id: "demo2", fullName: "Amit Patel", email: "amit@example.com", whatsapp: "919876543211", platformId: "pinup", platformName: "Pin-Up Casino", trafficSource: "YouTube", estimatedMonthlyPlayers: "500+", status: "pending", appliedAt: (/* @__PURE__ */ new Date()).toISOString() },
      { id: "demo3", fullName: "Vikram Singh", email: "vikram@example.com", whatsapp: "919876543212", platformId: "1xbet", platformName: "1xBet", trafficSource: "SEO Blog", estimatedMonthlyPlayers: "50+", status: "approved", appliedAt: new Date(Date.now() - 864e5).toISOString() }
    ];
  }
  res.json({
    isDemo,
...
var subPartnerLimiter = (0, import_express_rate_limit.default)({ windowMs: 60 * 60 * 1e3, max: 5 });
...
app.post("/api/admin/custom-pages", verifyJwtToken, import_express.default.json({ limit: "50mb" }), (req, res) => {
...
var trackLimiter = (0, import_express_rate_limit.default)({ windowMs: 1 * 60 * 1e3, max: 30 });
app.post("/api/track", trackLimiter, (req, res) => {
  ...
  if (eventType === "click") {
    ...
    const today = (/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { month: "short", day: "numeric" });
...
async function startServer() {
  ...
  if (!isProduction) {
    ...
    const candidates = [
      ...
      import_path.default.join(_dirname),
      process.cwd()
    ];
    ...
    if (fs.existsSync(publicPath)) {
      app.use(import_express.default.static(publicPath, { maxAge: "1d" }));
    }
