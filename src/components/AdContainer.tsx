...
export const AdContainer: React.FC<AdContainerProps> = ({ slotId, className = '' }) => {
  return (
    <div className={`w-full flex flex-col items-center justify-center my-4 ${className}`}>
      <span className="text-[10px] text-slate-400 uppercase tracking-widest mb-1">Advertisement</span>
      <div 
        ...
        data-ad-slot={slotId}
      >
        <span className="text-slate-400 text-sm font-medium group-hover:text-amber-400 transition-colors">
          Ad Space ({slotId})
        </span>
